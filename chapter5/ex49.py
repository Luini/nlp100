...
if __name__ == "__main__":
    ...
    for path, path2 in pathPairs:
        if path2[0] in path:
            ...
            path = [
                chunk.getSurface() if index != 0 and index != len(path)-1 else chunk.replaceNoun("X") if index == 0 else chunk.replaceNoun("Y")
                for index, chunk in enumerate(path)
            ]
            print(" -> ".join(path))
        else:
            (commonPartIndex, commonPartIndex2) = getCommonPartIndex(path, path2)
            if commonPartIndex:
                result = [path[:commonPartIndex], path2[:commonPartIndex2], path[commonPartIndex:commonPartIndex+1]]
